...
class GameView(arcade.View):
    ...
    def __init__(self, xray):
        ...
        # init vision
        self.vision = [[0 for i in range(10)] for j in range(10)]
    ...
    def on_draw(self):
        ...
        arcade.draw_text(output_battery, 10, 30, arcade.color.WHITE, font_size)

        output_minerals = f'Minerals: {self.mineral_count}'
        arcade.draw_text(output_minerals, 10, 5, arcade.color.WHITE, font_size)
    ...
    def on_key_press(self, key, key_modifiers):
        ...
        if key == arcade.key.S:
            ...
            if self.y - 1 >= 0:
                self.vision[self.x][self.y - 1] = 1
            print('action: scan', '\tloc: ', self.x, self.y, '\tbattery: ', self.battery,  '\tminerals: ', self.mineral_count, '\t\tminerals left: ', self.minerals[self.x][self.y])
